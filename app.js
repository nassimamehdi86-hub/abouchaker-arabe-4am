...
async function renderAdminPanel(){
  ...
  const approvedNames = await Admin.listApproved();
  html += `<div class="section-title" style="margin-top:22px"><span>التلاميذ المقبولون (${approvedNames.length})</span><div class="line"></div></div>`;
  if(!approvedNames.length){
    html += `<div class="exam-panel">لا يوجد بعد أي تلميذ مقبول.</div>`;
  } else {
    html += `<div class="lesson-list">` + approvedNames.map((n,i)=>`
      <div class="lesson-row">
        <div class="lr-num">${i+1}</div>
        <div class="lr-text"><div class="lr-title">${n}</div></div>
      </div>`).join('') + `</div>`;
  }
