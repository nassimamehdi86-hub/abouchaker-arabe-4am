...
async function renderLessonExercisesBox(lesson){
  ...
  box.innerHTML = `
    <div class="one-attempt-warn">
      ...
      <div>
        ...
        <div class="oaw-sub">عدد الأسئلة: ${data.questions.length}. لا يمكنك إعادة هذا التمرين بعد إرساله، وتُحسب نتيجتك بالنسبة المئوية وتدخل ترتيب هذا الدرس.</div>
      </div>
    </div>
  ...
  document.getElementById('ldExerciseStartBtn').addEventListener('click', ()=>{
    ...
    createExerciseEngine(lesson, data.questions, document.getElementById('ldExerciseMount'));
  });
}

/* ---------- محرك «تمارين الدرس» — محاولة واحدة، بلا إعادة، يُسجَّل في الترتيب ---------- */
function createExerciseEngine(lesson, questions, mountEl){
  ...
  function renderQuestion(){
    ...
    mountEl.querySelector('.quiz-next-btn').addEventListener('click', ()=>{
      idx++;
      if(idx >= total) finish(); else renderQuestion();
    });
  }
