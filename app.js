...
/* =========================================================================================
   ...
   ========================================================================================= */
const Student = {
  id:null, fullName:null, status:null, sessionId:null, unsubscribe:null,

  normalizedKey(fullName){ return normalizeAr(fullName); },

  /* محاولة الدخول أو التسجيل بالاسم واللقب */
  async loginOrRegister(fullName){
...
const Admin = {
  ...
  async approve(id){ if(fbReady) await db.collection('students').doc(id).update({status:'approved'}); },
  async reject(id){ if(fbReady) await db.collection('students').doc(id).update({status:'rejected'}); },
...
function createQuizEngine(lesson, mountEl){
  ...
  function renderQuestion(){
    if(idx >= roundQueue.length){
      // انتهت الجولة: احسب واعرض النتيجة
      renderResult();
      return;
    }
...
function renderLessonsScreen(){
  ...
  Locks.load().then(()=>{
    let html = '';
    ['tawabi','qawaid','jumal'].forEach(cat=>{
...
function openLessonDetail(id){
  ...
  document.getElementById('ldVideo').src = `https://www.youtube.com/embed/${videos[0].yt}?rel=0`;
...
async function renderAdminPanel(){
  ...
  if(!pending.length){
    ...
    html += `<div class="lesson-list">` + pending.map(p=>`
      <div class="lesson-row">
        <div class="lr-text"><div class="lr-title">${p.fullName}</div></div>
        <button class="al-key" style="width:auto;padding:6px 14px" data-approve="${p.id}">✅ قبول</button>
        <button class="al-key" style="width:auto;padding:6px 14px" data-reject="${p.id}">❌ رفض</button>
      </div>`).join('') + `</div>`;
  }
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  document.getElementById('loginSubmitBtn').addEventListener('click', async ()=>{
    ...
    if(!name){ alert('يرجى كتابة الاسم واللقب.'); return; }
    const btn = document.getElementById('loginSubmitBtn');
    btn.disabled = true; btn.textContent = 'جارٍ التحقق…';
    const res = await Student.loginOrRegister(name);
