...
function renderLessonsScreen(){
  ...
  Locks.load().then(()=>{
    let html = '';
    /* ===== سطر تشخيصي مؤقت: يعرض البيانات الفعلية القادمة من قاعدة البيانات =====
       احذف هذا الـ <div> بعد التأكد من حل المشكلة، فهو موجود فقط لتشخيصها الآن */
    html += `<div style="background:#fff3cd;border:2px solid #c0392b;border-radius:10px;padding:10px;margin-bottom:14px;font-size:11px;direction:ltr;text-align:left;word-break:break-all;color:#000">
      DEBUG fbReady=${fbReady} | lessons=${JSON.stringify(Locks.data.lessons || {})}
    </div>`;
