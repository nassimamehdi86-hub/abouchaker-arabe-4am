...
const LocksEnhanced = {
  ...
  async setLessonWithNotification(id, open, lessonTitle) {
    ...
    if (open && NotificationsSystem) {
      await NotificationsSystem.addNewContentAlert('lesson', lessonTitle, 'متاح الآن للتلاميذ');
    }
  }
};
...
function setupLessonLiveUpdates() {
  ...
  Locks.listen(() => {
    /* إعادة تحديث قائمة الدروس إذا كان الطالب يشاهدها */
    if (Screens.current === 'lessons' && typeof renderLessonsList === 'function') {
      renderLessonsList();
    }
  });
}
...
function initNotificationsSystem() {
  ...
  if (bellIcon && notificationsPanel) {
    if (typeof NotificationsSystem !== 'undefined') {
      NotificationsSystem.init(bellIcon, notificationsPanel);
...
async function renderStudentManagementPanel() {
  ...
  document.getElementById('sendNotificationBtn').addEventListener('click', async () => {
    ...
    try {
      await NotificationsSystem.sendNotification(title, message, '📢');
...
async function enhancedLessonOpen(lessonId, lessonTitle) {
  ...
  if (typeof renderLessonsList === 'function') {
    renderLessonsList();
  }
}
