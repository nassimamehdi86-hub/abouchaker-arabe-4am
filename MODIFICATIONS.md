...
## تحديث — تبسيط صفحة الدروس (قائمة واحدة بلا مجموعات)

- `app.js` (`renderLessonsScreen`): أُلغيت المجموعات (التوابع، قواعد اللغة، …). كل الدروس المفتوحة في قائمة واحدة، وتحت اسم كل درس اسم وحدته. تبقى "المكتسبات القبلية" وحدها داخل مجموعة قابلة للفتح في آخر الصفحة.
- ترتيب القائمة: آخر درس يفتحه الأستاذ يظهر أولًا. يُسجَّل وقت الفتح في `state/locks.openedAt[lessonId]` (`Locks.setLesson` و`LocksEnhanced.setLessonWithNotification`). الدروس المفتوحة قبل هذا التحديث تأتي بعد الأحدث، الأعلى ترتيبًا في المنهج أولًا. دروس "قريبًا" في الآخر.
- لا حاجة لأي تغيير في قواعد Firestore.
- شارة "جديد" (`.lr-new-badge`) على أحدث درس مفتوح فقط، وتزول بعد 7 أيام من فتحه.

## تحديث — نوافذ صفحة الدرس بنفس بطاقات الرئيسية
- `app.js` (`openLessonDetail` + `setupLdTabs`): تُرسم نوافذ الدرس (تمارين الدرس، حصص الزوم، اختبار الفهم، الخريطة الذهنية) كبطاقات `.home-card` (أيقونة + عنوان + سطر فرعي) بدل الأزرار الصغيرة.
- `style.css`: `.ld-tabs` شبكة عمودين بفجوة 14px مثل `.home-menu`، و`.ld-tab-card.active` لتمييز النافذة المفتوحة. أُزيل `.ld-tab-btn`.
